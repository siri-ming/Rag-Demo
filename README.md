...
![文档管理](docs/images/a.png)

- 拖拽上传或点击选择文件
- 支持 PDF、Word、Excel、Markdown、纯文本、图片等格式
- 自定义知识库名称、分块策略和参数
- 查看已上传文档列表，支持预览源文件和分块详情

### 智能问答界面

![智能问答](docs/images/b.png)
...
本系统集成了 **RapidOCR**（基于 ONNX Runtime）实现图片文字识别：

- **自动触发**：上传图片（PNG/JPG/JPEG）时自动调用 OCR 引擎提取文字
- **PDF 内嵌图片**：扫描版 PDF 中的图片页面同样通过 OCR 提取文本
- **多语言支持**：支持中英文混合识别
- **高精度**：基于深度学习模型，识别准确率可达 95%+

> **依赖安装**：`rapidocr-onnxruntime` 已在 `pyproject.toml` 中声明，执行 `uv sync` 即可自动安装。
